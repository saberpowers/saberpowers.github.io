...
\begin{document}
  ...
  \begin{framed}
    ...
    Check out the FanGraphs WPA Inquirer at fangraphs.com/tools/wpa-inquirer.
    \begin{itemize}
      \item Imagine you are the home team in a tied ballgame in the bottom of the ninth inning. You have a runner on second base and nobody out. Suppose that you could choose to bunt, in which case (we will assume) you would have a runner on third base one one out. Would this be a good idea?
      \item Try to find the scenario in which it is most advantageous to bunt (assume runners on first base and second base advance one base each and there is one additional out). For which game scenario would a bunt increase the win probability by the most?
    \end{itemize}
  \end{framed}
  ...
  \section{\sc Player Evaluation and In-Game Strategy}
    ...
    \subsection{\sc Discussion}
      ...
      \begin{enumerate}
        \item How would personnel considerations (i.e., the players involved in a play) change how you would use a Markov chain model to make an in-game strategic decision?
      \end{enumerate}
