...
\begin{framed}
  \textsc{3-Point Summary}
  \begin{enumerate}
    \item Per the Pythagorean formula, wins come from runs. But where do runs come from?
    \item Linear Weights (LW) are more stable than RE24 because they strip away context.
    \item Choosing between low bias and low variance is a false dichotomy (regression to the mean!).
  \end{enumerate}
\end{framed}
  ...
  \section{\sc Base-Out Run Expectancy and Linear Weights}
    ...
    \subsection{\sc Discussion Questions}
      ...
      \begin{enumerate}
        ...
        \item Navigate to the FanGraphs Major League Leaders page for batting. Compare the ranking of the top players by RE24 (under the Win Probability tab) and wRAA (under the Advanced tab) for the current season. Which players stand out as being ranked differently on these two leaderboards? What does that say about those players?
        \item Navigate to the FanGraphs Projected Standings page for the current season. Which teams have the biggest differences between their Pythagorean records and their BaseRuns records, in both directions? What does that say about those teams?
      \end{enumerate}
